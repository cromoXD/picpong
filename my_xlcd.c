 ...
 *			   A função utilizada foi a Delay1KTCYx() da bilioteca padrão C18
...
//Envio dos comandos de Configuração do LCD
	XLCD_executar(0b00101000);	// Function Set: 4bit interface, 2 lines, Font 5x7  
//    XLCD_executar(0b00100100);	// Function Set: 4bit interface, 2 lines, Font 5x10  
	XLCD_executar(0b00000110);	// Entry mode set: Increment, Shift OFF
	XLCD_executar(0b00001100);	// Display Control: Display ON, Cursor OFF, Blink OFF
	XLCD_limpar();
}
 ...
 *****************************************************************************/
void XLCD_executar(unsigned char cmd)
{
	//Seleciona pinos de Controle para escrita do comando
	//não usado // 	RW_PIN = 0;	//escrita=0, leitura=1
	RS_PIN = 0;		// comando=0, dados=1
 ...
 *****************************************************************************/
void XLCD_escrever_byte(char data)
{
	//Seleciona pinos de Controle para escrita do comando
	//não usado // 	RW_PIN = 0;	//escrita=0, leitura=1
 ...
 *****************************************************************************/
void XLCD_posicionar(unsigned char DDaddr)
{
	//Seleciona pinos de Controle para escrita do comando
	//não usado // 	RW_PIN = 0;	//escrita=0, leitura=1
	RS_PIN = 0;		// comando=0, dados=1
 ...
 *				SetDDRamAddr().
 *****************************************************************************/
void XLCD_escrever_string(char *buffer)
{
        while(*buffer)                  // escreve dados no LCD ate achar caracter nulo
        {
                XLCD_escrever_byte(*buffer); // escreve caracter no LCD
 ...
 *				SetDDRamAddr().
 *****************************************************************************/
void XLCD_escrever_rom_string(const rom char *buffer)
{
        while(*buffer)                  // escreve dados no LCD ate achar caracter nulo
        {
                XLCD_escrever_byte(*buffer); // escreve caracter no LCD
 ...
 * Limpa a tela
 */
void XLCD_limpar(void)
{
		XLCD_executar(0b00000001);
}
 
/**
 * Inicia a escrita na CGRAM
 */
void XLCD_CGRAM(void)
{
		XLCD_executar(0b01000000); 
}
